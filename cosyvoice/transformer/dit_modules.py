...
class SinusPositionEmbedding(nn.Module):
    ...
    def forward(self, x, scale=1000):
        try:
            ...
            data = torch.tensor(x).unsqueeze(0).to(torch.float32)
            data = data.view(data.size(0),1)        
        device = x.device
        half_dim = self.dim // 2
        emb = math.log(10000) / (half_dim - 1)
        emb = torch.exp(torch.arange(half_dim, device=device).float() * -emb)
        emb = scale * data * emb.unsqueeze(0)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if c is not None:
            return self.processor(self, x, c=c, attn_mask=attn_mask, mask=mask, rope=rope, c_rope=c_rope)
        else:
            return self.processor(self, x, attn_mask=attn_mask, mask=mask, rope=rope)
...
class SteamingAttnProcessor:
    ...
    def __call__(
        ...
        # if mask is not None:
        #     attn_mask1 = mask
        #     attn_mask1 = attn_mask1.unsqueeze(1).unsqueeze(1)  # 'b n -> b 1 1 n'
        #     attn_mask1 = attn_mask1.expand(batch_size, attn.heads, query.shape[-2], key.shape[-2])
        #     attn_mask1 = torch.logical_and(block_attn_mask, attn_mask1)
        #     attn_mask = attn_mask.unsqueeze(1)
        #     attn_mask = attn_mask.expand(attn_mask.size(0), attn.heads, attn_mask.size(-2), attn_mask.size(-1))
        # else:
        #     attn_mask1 = block_attn_mask
        attn_mask = attn_mask.unsqueeze(1)
        attn_mask = attn_mask.expand(attn_mask.size(0), attn.heads, attn_mask.size(-2), attn_mask.size(-1))
        # print("attention mask : {}|{}".format(attn_mask.shape, attn_mask1.shape))
        # [t, t, t, f, f, ]
        # [t, t, t, t, t, ]
        x = F.scaled_dot_product_attention(query, key, value, attn_mask=attn_mask, dropout_p=0.0, is_causal=False)
        x = x.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, t, mask=None, attn_mask=None, rope=None):  # x: noised input, t: time embedding
        # pre-norm & modulation for attention input
        norm, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.attn_norm(x, emb=t)

        # attention
        attn_output = self.attn(x=norm, attn_mask=attn_mask, mask=mask, rope=rope)
